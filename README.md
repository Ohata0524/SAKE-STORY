...
## ディレクトリ構成
SAKE-STORY/
├── app/                      # App Router：メインのアプリケーションロジック
│   ├── lib/                  # ユーティリティ・共通処理
│   │   └── supabaseClient.js # Supabaseクライアントの設定
│   ├── list/                 # 銘柄一覧画面
│   │   ├── [id]/             # 銘柄詳細画面（動的ルーティング）
│   │   │   └── page.tsx
│   │   └── page.tsx
...
│   │   └── page.tsx
│   ├── favicon.ico           # サイトアイコン
│   ├── globals.css           # 全体共通のCSSスタイル（Tailwind CSS）
│   ├── layout.tsx            # 全画面共通のレイアウト定義
│   └── page.tsx              # トップページ（LPなど）
├── docs/                     # ドキュメント・資料
│   └── images/               # README用などのスクリーンショット
├── public/                   # 静的アセット（画像、フォント等）
├── .env.local                # 環境変数（認証キー、DB接続情報等）
├── .gitignore                # Git管理から除外するファイルの設定
├── eslint.config.mjs         # Linter（コード規約）の設定
